...
function ConnectionManager(server, subscriber) {
  ...
  setInterval(function() {
    ...
    if (!this.activeUsers_[davidben]) {
      ...
      console.log('==== davidben has %d active sockets and %d active tails',
...
ActiveUser.prototype.addSocket = function(socket) {
  this.ref_++;
  if (this.user_.id === 1)
    console.log('==== davidben opened a socket (ref = %d)', this.ref_);
  ...
  socket.on('close', function() {
    ...
    if (this.user_.id === 1)
      console.log('==== davidben closed a socket (ref = %d)', this.ref_);
  }.bind(this));
...
Tail.prototype.fireQuery_ = function() {
  ...
  this.messageBuffer_ = [];
  // Make the DB query.
  db.getMessages(this.user_.user_, this.lastSent_, this.filter_, {
