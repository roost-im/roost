...
function ConnectionManager(server, subscriber) {
  ...
  this.io_.set('authorization', function(handshakeData, cb) {
    var token = handshakeData.query.access_token;
    if (!token) {
      cb(null, false);
      return;
    }
    auth.checkAuthToken(token).then(function(user) {
      // This apparently works, although it's really rather
      // terrible. Judging by the code, the callback is supposed to
      // take a third newData argument, but it doesn't actually work.
...
ConnectionManager.prototype.onSocket_ = function(socket) {
  ...
  this.activeUsers_[user.id].addSocket(socket);
};
