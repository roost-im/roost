...
Subscriber.prototype.addUserSubscription = function(user, sub, creds) {
  ...
  if (zuser.isPersonal(sub[2])) {
    ...
    // TODO(davidben): Instead of adding these one at a time, better
    // to re-subscribe to all of them or something.
    //
    // TODO(davidben): Check that the creds we got make any sense.
    return Q.all([
      db.addUserSubscription(user, sub[0], sub[1], sub[2]),
      innerDemon.subscribeTo([zephyrSub], creds)
    ]);
  }
  return Q.all([
    db.addUserSubscription(user, sub[0], sub[1], sub[2]),
    Q.nfcall(zephyr.subscribeTo, [zephyrSub])
  ]);
};
...
Subscriber.prototype.removeUserSubscription = function(user, sub, creds) {
  ...
  return db.removeUserSubscription(user, sub[0], sub[1], sub[2]);
};
