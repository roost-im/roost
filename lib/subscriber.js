...
function Subscriber() {
  ...
  // Current pending state-saving job.
  this.savingState_ = null;
...
Subscriber.prototype.start = function() {
  ...
  return renewPromise.then(function() {
    ...
    this.restoreInnerDemonState_().then(function() {
      ...
      this.stateTimer_ = setInterval(function() {
        this.saveInnerDemonState_().then(null, function(err) {
          console.error('Error saving inner demon state', err);
        }).done();
      }.bind(this), conf.get('demonStateTimer'));
    }.bind(this), function(err) {
...
Subscriber.prototype.addUserSubscriptions = function(user, subs, creds) {
  ...
  for (var i = 0; i < subs.length; i++) {
    if (zuser.isPersonal(subs[i].recipient)) {
  ...
  return db.addUserSubscriptions(user, subs).then(function(withKeys) {
    ...
    // downcase everywhere.
    var publicSubs = [], hasPersonals = false;
    for (var i = 0; i < withKeys.length; i++) {
      if (zuser.isPersonal(withKeys[i].recipient)) {
        hasPersonals = true;
      } else {
    ...
    return Q.all([
      // Personals, if any.
      hasPersonals ? this.refreshPrivateSubs(user, creds) : Q(),
      // Public notices, if any.
      publicSubs.length ?
        Q.nfcall(zephyr.subscribeToSansDefaults,
                 publicSubs.map(subscriptionToZephyrTriple)) : Q()
    ]).then(function() {
      return withKeys;
    });
  }.bind(this));
...
Subscriber.prototype.saveInnerDemonState_ = function(forceNew) {
  // Meh. There's an fs.writeFile in there. May as well not be racy or
  // something... Although with a half-hour timeout, this shouldn't
  // happen.
  if (this.savingState_) {
    console.log('(Already pending inner demon save)');
    if (forceNew) {
      // We're shutting down. Wait for that one to finish and do it again.
      return this.savingState_.then(function() {
        return this.saveInnerDemonState_(false);
      }.bind(this), function(err) {
        return this.saveInnerDemonState_(false);
      }.bind(this));
    } else {
      return this.savingState_;
    }
  }

  // TODO(davidben): Probably also good to have this trigger when the
  // user subscribes somewhere.
  console.log('Querying inner demons for session state');
  this.savingState_ = this.getInnerDemonState().then(function(state) {
...
Subscriber.prototype.shutdown = function() {
  return this.saveInnerDemonState_(true).then(function() {
