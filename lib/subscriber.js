...
Subscriber.prototype.processMessage_ = function(msg) {
  ...
  return this.dbPool_.getConnection().then(function (conn) {
    return conn.saveMessage(
      msg
    ).finally(conn.end.bind(conn)).then(function(ret) {
...
Subscriber.prototype.addDefaultUserSubscriptions = function(user) {
  ...
  return this.addUserSubscriptions(user, subs, null, true).then(function() {
    // Throw away the return...
  });
};

Subscriber.prototype.addUserSubscriptions = function(user,
                                                     subs,
                                                     creds,
                                                     allowNoCreds) {
  // ACL checks.
