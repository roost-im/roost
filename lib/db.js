...
function findSubscription(conn, klass, inst, recip) {
  ...
  return Q.ninvoke(conn, 'query',
                   ...
                   [klass, inst, recip]).then(function(args) {
    var rows = args[0];  // Q turns arguments into arrays.
    if (rows.length === 0)
      return null;
    return rows[0].id;
  });
...
exports.addSubscription = function(user, klass, inst, recip) {
  throw "Not implemented";
};
