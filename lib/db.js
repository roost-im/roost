...
Connection.prototype.getUser = function(principal) {
  return this.query(
    'SELECT id, principal FROM users WHERE principal = ? LIMIT 1', [principal]
  ).then(function(rows) {
    if (!rows.length)
      return null;
    return {
...
Connection.prototype.addUserSubscription = function(user, klass, inst, recip) {
  ...
  var sub = {
    ...
    instance: new Buffer(inst),
    recipient: new Buffer(recip),
    class_key: new Buffer(klassKey),
    instance_key: new Buffer(instKey)
  };
  // TODO(davidben): Instead of IGNORE, do ON DUPLICATE KEY UPDATE for
  // class and instance? We can change the user-visible name.
  return this.query(
    'INSERT IGNORE INTO subs SET ?', [sub]
  ).then(function(result) {
    return {
      class: klass,
      instance: inst,
      ...
      classKey: klassKey,
      instanceKey: instKey,
      newSubscription: (result.insertId != 0)
    };
  });
};
